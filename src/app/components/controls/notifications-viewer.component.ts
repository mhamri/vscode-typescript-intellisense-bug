...
export class NotificationsViewerComponent implements OnInit, OnDestroy {
  ...
  initDataLoading() {
    ...
    const dataLoadTask = this.isViewOnly
  ...
  getPrintedDate(value: Date) {
    if (value) {
      return (
        Utilities.printTimeOnly(value) + ' on ' + Utilities.printDateOnly(value)
      );
    }
  }
  ...
  deleteNotification(row: Notification) {
    this.alertService.showDialog(
      'Are you sure you want to delete the notification "' + row.header + '"?',
      DialogType.confirm,
  ...
  deleteNotificationHelper(row: Notification) {
    ...
    this.notificationService.deleteNotification(row).subscribe(
      (results) => {
  ...
  togglePin(row: Notification) {
    ...
    this.notificationService.pinUnpinNotification(row, pin).subscribe(
      (results) => {
        this.alertService.stopLoadingMessage();
        this.loadingIndicator = false;
